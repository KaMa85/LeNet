...
# Define the ImmerseNet0 model
class ImmerseNet0(nn.Module):
    def __init__(self):
        super(ImmerseNet0, self).__init__()
...
# Create the model
model = ImmerseNet0()
model.load_state_dict(torch.load('ImmerseNet0_weights.pth'))
...
public static class ImmerseNet0Weights
{
"""
...
"""
        ...
        for i, sub_param in enumerate(param):
            ...
            array_str = ', '.join(map(str, flat_param))
            csharp_code += f"""
    float[] {name.replace('.', '_')}_{i} = new float[]
    {{
        {array_str}
    }};
"""
...
with torch.no_grad():
    for data in test_loader:
        ...
        # Collect the first element of the inputs in each batch
        first_elements.append(inputs[0].numpy())

# Write the collected first elements to a C# file
with open('first_elements.cs', 'w') as file:
    file.write('// This file contains the first elements of inputs for each batch as 1D arrays\n\n')
    for i, element in enumerate(first_elements):
        array_string = ", ".join(map(str, element.flatten()))
        file.write(f'float[] array{i} = {{{array_string}}};\n')
