...
class ImmerseNet_Segmentation(nn.Module):
    def __init__(self):
        super(ImmerseNet_Segmentation, self).__init__()
        ...
        self.conv_transpose2 = nn.ConvTranspose2d(32, 16, kernel_size=2, stride=2)
        self.conv_transpose3 = nn.ConvTranspose2d(16, 1, kernel_size=2, stride=2)
    ...
    def forward(self, x):
        ...
        x = self.conv_transpose3(x)  # No activation after the final layer
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        for j, batch in enumerate(eval_loader):
            ...
            inputs = inputs.float()
            labels = labels.long()
            labels = labels.unsqueeze(1)  # Now labels should be [16, 1, 40, 40]
    ...
    if avg_ep_val_loss < best_loss:
        ...
        torch.save(model.state_dict(), 'ImmerseNet_Segmentation_weights.pth')
...
public static class ImmerseNet_SegmentationWeights
{
"""
...
with open('ImmerseNet_SegmentationWeights.cs', 'w') as f:
    f.write(csharp_code)
