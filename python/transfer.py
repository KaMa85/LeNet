...
# Plotting one positive and one negative example from both training and testing sets
def plot_sample_images(train_dataset, test_dataset):
    # Find one positive and one negative sample in the training set
    train_pos_sample = next((img for img, label in train_dataset if label == 1), None)
    train_neg_sample = next((img for img, label in train_dataset if label == 0), None)

    # Find one positive and one negative sample in the testing set
    test_pos_sample = next((img for img, label in test_dataset if label == 1), None)
    test_neg_sample = next((img for img, label in test_dataset if label == 0), None)

    # Create subplots
    fig, axs = plt.subplots(2, 2, figsize=(10, 10))

    # Plot training set samples
    axs[0, 0].imshow(train_pos_sample.squeeze(), cmap='gray')
    axs[0, 0].set_title('Training Set - Positive Sample')
    axs[0, 0].axis('off')

    axs[0, 1].imshow(train_neg_sample.squeeze(), cmap='gray')
    axs[0, 1].set_title('Training Set - Negative Sample')
    axs[0, 1].axis('off')

    # Plot testing set samples
    axs[1, 0].imshow(test_pos_sample.squeeze(), cmap='gray')
    axs[1, 0].set_title('Testing Set - Positive Sample')
    axs[1, 0].axis('off')

    axs[1, 1].imshow(test_neg_sample.squeeze(), cmap='gray')
    axs[1, 1].set_title('Testing Set - Negative Sample')
    axs[1, 1].axis('off')
...
positive_images = load_images_from_folder(
    r'C:\!\Dessertation\Technical\AI\after_last\ImmerseNet\transfer\data\mss_ps_55_227\pos')
...
negative_images = load_images_from_folder(
    r'C:\!\Dessertation\Technical\AI\after_last\ImmerseNet\transfer\data\mss_ps_55_227\neg')
...
# Hyperparameters
batch_size = 4  # Set the batch size here
learning_rate = 0.001
num_epochs = 100
pooling_kernel_size = 2  # Define the pooling kernel size
pooling_stride = 2  # Define the pooling stride

# Print the pooling parameters
...
# Convert datasets to sets of indices
...
if overlap:
    print(f'Overlap detected: {len(overlap)} overlapping items.')
else:
...
plot_sample_images(train_dataset, test_dataset)
...
# Create the model
model = ImmerseNet1()
model.load_state_dict(torch.load('ImmerseNet1_weights.pth'))

# Freeze all layers except the last few (customize as per your model's architecture)
for param in model.parameters():
    param.requires_grad = False

# Assuming the last layer or layers are fully connected layers for classification
# Unfreeze the last layer (or a few last layers) by setting requires_grad=True
...
for epoch in range(num_epochs):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        if (i + 1) % (batch_size * 10) == 0:
            print('[Epoch %d, Batch %5d] loss: %.3f' % (epoch + 1, i + 1, running_loss / (batch_size * 10)))
            running_loss = 0.0

    # Print total loss for the epoch
    print('[Epoch %d] total loss: %.3f' % (epoch + 1, total_loss / len(train_loader)))

    # Save the model weights if training loss improves
    if total_loss < best_loss:
        best_loss = total_loss
        torch.save(model.state_dict(), 'ImmerseNet1_weights.pth')
        print('Model saved with total loss: %.3f' % best_loss)
...
public static class ImmerseNet1TransferWeights
{
"""
...
with open('ImmerseNet1TransferWeights.cs', 'w') as f:
    f.write(csharp_code)
